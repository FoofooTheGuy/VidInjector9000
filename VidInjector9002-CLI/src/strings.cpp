...
std::string Version = "VidInjector9000 ver. 3.0.3-CLI";
...
std::string BuildC2InfoText = "The same as the previous one, except with the following defaults where the argument parameters are missing: (random unique ID) 'video' 'VDIJ'";
std::string ExCInfoText = "Extract parameters from a .cia to a directory which will contain the romfs directory, exefs directory, and .vi9p file. Note that <output directory> must be a complete path like \"C:/dir/dir\" and unnecessary files will not be extracted.";
