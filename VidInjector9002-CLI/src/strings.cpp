...
std::string BuildC3InfoText = "The same as the previous one, except with an output .tar file which will contain a luma game patch. (Only use this if INT:SPLITPATCH is set in the input .vi9p)";
std::string ExCInfoText = "Extract parameters from a .cia to a directory which will contain the romfs directory, exefs directory, and .vi9p file. Note that <output directory> must be an absolute path and unnecessary files will not be extracted.";
