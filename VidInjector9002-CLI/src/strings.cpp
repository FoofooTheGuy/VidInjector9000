...
std::string SetPInfoText = "Set a parameter based on which number of parameter to set. (Do -pp to see a list of possible numbers.) (Output file can be the same as input file)";
std::string SetSInfoText = "Similar to -sp, this reads the titles from an SMDH and sets them to short name, long name, and publisher in the output .vi9p file. Useful for when first using an SMDH for the icon (Do -pp to see a list of possible numbers.) (Output file can be the same as input file)";
...
std::string BuildC3InfoText = "The same as the previous one, except with an output .tar which will contain a Luma LayeredFS game patch. (Only use this if INT:SPLITPATCH is set in the input .vi9p)";
